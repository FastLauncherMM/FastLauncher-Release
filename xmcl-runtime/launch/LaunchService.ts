...
export class LaunchService extends AbstractService implements ILaunchService {
  ...
  async #generateOptions(options: LaunchOptions, version: ResolvedVersion, accessToken?: string) {
    ...
    const launcherName = `FastLauncher (${this.app.version})`
  ...
  async launch(options: LaunchOptions) {
    ...
    try {
      ...
      if ('inheritances' in version) {
        ...
        const independent = middlewares.filter(m => m.name === 'check-assets' || m.name === 'check-natives')
        const dependent = middlewares.filter(m => m.name !== 'check-assets' && m.name !== 'check-natives')
        await Promise.all(independent.map(async (plugin) => {
          try {
  ...
  async #persistAbnormalExitLog(gameDirectory: string, info: {
    ...
    const header = [
      `# FastLauncher abnormal-exit dump`,
