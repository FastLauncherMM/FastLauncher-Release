...
async function downloadAsarUpdate(
  ...
  const urls = gfw.inside
    ? [`https://github.com/FastLauncherMM/FastLauncher-Release/releases/download/v${version}/${file}`]
    : [`https://github.com/FastLauncherMM/FastLauncher-Release/releases/download/v${version}/${file}`]
...
async function downloadAppInstaller(
  ...
  const destination = join(app.getPath('downloads'), 'FastLauncher.appinstaller')
      ...
      Object.assign(executor, {
        ...
        createRequest: (options: any, callback: any) => {
          if (gfw.inside) {
            ...
            options.pathname = `/Soft_Mirrors/github-release/FastLauncherMM/FastLauncher-Release/LatestRelease/${basename(options.pathname)}`
