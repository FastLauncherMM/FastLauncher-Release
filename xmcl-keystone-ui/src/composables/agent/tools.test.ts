...
import { createFlTools, buildSystemPrompt } from './tools'

// `createFlTools` pulls services via `useService`. Mock the module so the
...
describe('createFlTools — registry composition', () => {
  test('base toolset includes diagnose_server and the shader-options tools', () => {
    const names = createFlTools(makeAgentContext()).base.map((t) => t.name)
  ...
  test('lazy packs are market, troubleshoot, server, worlds, instance_admin, account', () => {
    const reg = createFlTools(makeAgentContext())
...
describe('createFlTools — base tool behavior', () => {
  ...
  test('diagnose_server guards when no instance is selected', async () => {
    ...
    const reg = createFlTools(makeAgentContext({ getServerStatus, instance: ref({ path: '', runtime: {} }) as any }))
    const res = await getTool(reg.base, 'diagnose_server').execute({}, noopSignal)
  ...
  test('navigate rejects unknown routes and pushes known ones', async () => {
    const push = vi.fn()
    const reg = createFlTools(makeAgentContext({ router: { push } as any }))
...
describe('createFlTools — local server vfs (server/ subtree)', () => {
