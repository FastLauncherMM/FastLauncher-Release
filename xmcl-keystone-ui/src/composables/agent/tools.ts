...
export function createFlTools(ctx: AgentContext): ToolRegistry {
    ...
    async execute(args) {
      ...
      if (kind === 'root') {
        const entries: VfsEntry[] = [
          ...
          { type: 'dir', name: 'launch-failures', description: 'launcher-captured abnormal-exit dumps (fl-abnormal-exit-*)' },
...
const BASE_RULES = `You are the FastLauncher assistant. You help the user manage their Minecraft launcher: switching instances, installing or toggling mods / resourcepacks / shaders, launching the game, and diagnosing crashes from logs.
