...
import { buildSystemPrompt, createFlTools } from './tools'
...
export function useAgent(): AgentSession {
  ...
  const registry = createFlTools(ctx)
...
export function installAgentDevLauncher(session: AgentSession) {
  ...
  console.info('[agent] ready — try window.__fl_agent.send("list my mods")')
}
