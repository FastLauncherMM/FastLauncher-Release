...
/**
 * Exports all settings from localStorage into a downloadable .flsettings file.
 */
async function handleExportSettings() {
  try {
    ...
    a.href = url
    a.download = `fl-settings-${new Date().toISOString().split('T')[0]}.flsettings`
...
/**
 * Prompts the user to select a .flsettings file and imports the settings from it.
 */
async function handleImportSettings() {
  try {
    const input = document.createElement('input')
    input.type = 'file'
    input.accept = '.flsettings'
